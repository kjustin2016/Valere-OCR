...
def extract_information(text):
    ...
    for line in lines:
        ...
        if "Mother Name" in line:
            ...
            info["Physician Name"] = parts[0].split(":")[1].strip()
            if len(parts) > 1:
                info["Physician NPI"] = parts[1].strip()
                print(f"Extracted Physician NPI: {info['Physician NPI']}")  # Additional debug statement
            else:
                print("Physician NPI not found in line:", line)  # Additional debug statement
        elif "Date" in line and "Physician Signature" in line:
            info["Date"] = line.split(":")[2].strip()
